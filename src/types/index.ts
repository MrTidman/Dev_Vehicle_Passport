...
export interface Car {
  id: string;
  vin?: string | null;  // Only returned for admin/internal queries
  vin_last6: string | null;  // Masked VIN - last 6 characters only
  registration: string | null;
  make: string | null;
  model: string | null;
  year: number | null;
  fuel_type: string | null;
  engine_cc?: number | null;  // Not always returned
  colour: string | null;
  notes: string | null;
  shortcode: string | null;
  owner_id?: string;  // Not returned in user car queries for privacy
  created_by: string;
  created_at: string;
}
